class TicTacToeCardGame {
    ...
    resetBoard() {
        ...
        // Clear emote display
        const receivedEmoteDiv = document.getElementById('receivedEmote');
        if (receivedEmoteDiv) {
            receivedEmoteDiv.textContent = '';
        }
    ...
    updateScoreDisplay() {
        ...
        if (playerWinsLabel && opponentWinsLabel) {
            if (this.gameMode === 'online') {
                ...
                opponentWinsLabel.textContent = 'AI Wins:';
            } else {
                playerWinsLabel.textContent = 'Player Wins:';
    ...
    hasGameStateChanged(newState) {
        // Check if board, current player, game result, or emote changed
    ...
    getGameState() {
        return {
            ...
            myEmote: this.myEmote || null, // Store emote I sent
            receivedEmote: this.receivedEmote || null // Store emote received (will be synced separately)
        };
    }
    ...
    loadGameStateFromFirebase(gameState) {
        ...
        // Sync emotes - check if opponent sent a new emote
        // The opponent's myEmote becomes our receivedEmote
        const opponentEmote = gameState.myEmote; // This is what opponent sent (their myEmote)
        // Only display if it's different from what we last received and it's from the opponent
        if (opponentEmote && opponentEmote !== this.receivedEmote) {
            this.receivedEmote = opponentEmote;
            // Only display if game has ended (emotes are shown after game ends)
            if (!this.gameActive) {
                this.displayReceivedEmote(opponentEmote);
            }
        }
    ...
    displayReceivedEmote(emote) {
        ...
        if (receivedEmoteDiv) {
            // Add animation
            receivedEmoteDiv.style.animation = 'none';
            setTimeout(() => {
                receivedEmoteDiv.style.animation = 'popIn 0.3s ease';
            }, 10);
            
            receivedEmoteDiv.textContent = emote;
            
            // Clear after 5 seconds
            setTimeout(() => {
                if (receivedEmoteDiv.textContent.includes(emote)) {
                    receivedEmoteDiv.textContent = '';
                    this.receivedEmote = null;
                }
            }, 5000);
