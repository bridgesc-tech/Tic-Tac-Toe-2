class TicTacToeCardGame {
    ...
    updateCurrentPlayerDisplay() {
        ...
        if (this.gameMode === 'ai') {
            ...
            // Show which player's turn and indicate if it's your turn
            if (this.myPlayerColor) {
                const isMyTurn = this.currentPlayer === this.myPlayerColor;
                const playerText = this.currentPlayer === 'green' ? 'Green' : 'Red';
                if (isMyTurn) {
                    playerName.textContent = `Your Turn (${playerText})`;
                } else {
                    playerName.textContent = `${playerText} Player's Turn (Waiting...)`;
                }
            } else {
    ...
    createGameRoom() {
        ...
        this.gameRoomId = 'game_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
        ...
        db.collection('gameRooms').doc(this.gameRoomId).set({
            player1: 'green',
    ...
    joinGameRoom() {
        ...
        db.collection('gameRooms').doc(roomId).get().then(doc => {
            ...
            db.collection('gameRooms').doc(roomId).update({
                player2: 'red',
                status: 'active',
                gameState: roomData.gameState // Keep existing game state
            }).then(() => {
    ...
    setupFirebaseGameListener() {
        ...
        this.gameUnsubscribe = db.collection('gameRooms').doc(this.gameRoomId)
            .onSnapshot((doc) => {
                ...
                // Only update if it's not our own move (prevent infinite loop)
                if (gameState && this.gameMode === 'online') {
                    // Check if game state changed (prevent processing our own updates)
                    const stateChanged = this.hasGameStateChanged(gameState);
                    if (stateChanged) {
    ...
    hasGameStateChanged(newState) {
        // Simple check - compare board states
    ...
    loadGameStateFromFirebase(gameState) {
        // Don't update if it's our turn and we just made the move
        if (this.gameMode === 'online' && gameState.currentPlayer === this.myPlayerColor) {
            // This might be our own update, but we should still sync decks and board
            // Only sync visual state, don't change turn
            this.board = gameState.board || this.board;
            this.storedCellColors = gameState.storedCellColors || this.storedCellColors;
            this.playerDecks = gameState.playerDecks || this.playerDecks;
        ...
        // Check for game end
        if (!this.gameActive) {
            // Game ended - could show result if needed
        }
    }
